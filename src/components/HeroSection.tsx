...
const HeroSection = () => {
  ...
  return <section className="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden bg-gradient-to-b from-white to-gray-50">
      ...
      <div className="container mx-auto px-6 z-10">
        <div className="flex flex-col lg:flex-row items-center">
          <div className="lg:w-1/2 lg:pr-12 space-y-6 mb-10 lg:mb-0 text-center lg:text-left">
            ...
            <h1 className={`text-4xl md:text-5xl lg:text-6xl font-bold leading-tight text-balance transition-all duration-700 delay-100 transform ${isLoaded ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
              Delivering Packages,
              <br className="hidden sm:block" />
              <span className="text-primary">Preserving Planet</span>
            </h1>

            <p className={`text-lg text-gray-600 max-w-xl mx-auto lg:mx-0 transition-all duration-700 delay-200 transform ${isLoaded ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
            ...
            <div className={`flex flex-col sm:flex-row justify-center lg:justify-start space-y-4 sm:space-y-0 sm:space-x-4 transition-all duration-700 delay-300 transform ${isLoaded ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
              <Button onClick={handleSignUp} className="primary-button hover:scale-105">
                {isAuthenticated ? "Go to Dashboard" : "Sign Up Now"}
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button variant="outline" onClick={handleLearnMore} className="secondary-button hover:scale-105">
                Learn More
              </Button>
            </div>
          </div>

          <div className={`lg:w-1/2 relative transition-all duration-1000 delay-300 transform ${isLoaded ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"}`}>
            <div className="relative px-6 lg:px-0">
              ...
              <div className="glass rounded-2xl overflow-hidden relative z-10">
                <img alt="Eco-friendly delivery drone" className="w-full h-auto object-cover" loading="lazy" src="/lovable-uploads/8bc0ea83-3409-4e38-b509-11cf5cc7b1e4.jpg" />
              </div>
            </div>
          </div>
        </div>
